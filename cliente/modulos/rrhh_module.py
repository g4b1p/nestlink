...
ICON_POSTULANTES = customtkinter.CTkImage(
...
class RRHHModule(BaseAppWindow):
    ...
    ICON_POSTULANTE = ICON_POSTULANTES # Corregido para usar la variable de arriba
    ...
    def _ver_cv_postulante(self, candidato_id):
        ...
        try:
            ...
            # Usamos la nueva función helper de conexion_servidor para asegurar la URL correcta
            url_cv = conexion_servidor.get_cv_candidato(candidato_id)
...
class AgregarPostulanteModal(customtkinter.CTkToplevel):
    ...
    def __init__(self, master):
        ...
        # 🟢 CLAVE: Variable de instancia para guardar la ruta local del archivo
    ...
    def _seleccionar_cv(self):
        ...
        filepath = filedialog.askopenfilename(
            ...
            # 🟢 Ampliamos los tipos de archivo para incluir los permitidos en el backend (pdf, docx, img)
            filetypes=[
                ("Archivos de Documento", "*.pdf *.docx"),
    ...
    def _guardar_postulante(self):
        """Maneja el envío de datos y el CV al servidor (POST /api/candidatos)."""
        ...
        # 🟢 CLAVE: Llamar a la nueva función de conexión que maneja archivos
        try:
            success, message = conexion_servidor.post_nuevo_candidato(
                nombre=nombre,
                email=email,
                cv_filepath=self.cv_filepath
            )
            ...
            messagebox.showerror("Error Interno", f"Ocurrió un error inesperado al intentar guardar el candidato: {e}")
